...
extract = 'bhg_run_checkpoint extract_ipa unzip -q "${IPA}" -d "${VERIFY_ROOT}"'
...
for checkpoint in checkpoints:
    assert text.count(f'bhg_run_checkpoint {checkpoint} ') == 1
...
BHG_EXPECTED_CERT_SHA="$(printf 'fixture leaf certificate' | shasum -a 256 | cut -d ' ' -f 1)"
...
[[ "${BHG_APP_SIGNING_CERT_SHA}" == "${BHG_EXPECTED_CERT_SHA}" ]]
...
(
  set -euo pipefail
  bhg_run_checkpoint extract_ipa unzip -q "${BROKEN_IPA}" -d "${FIXTURE}/broken-out"
...
for checkpoint in locate_app verify_codesign extract_signing_certificate verify_certificate_continuity verify_arm64 verify_ipa_identity stage_result; do
  ...
  (
    set -euo pipefail
    bhg_run_checkpoint "${checkpoint}" bash -c 'printf "failure fixture-secret-protected-value https://private.invalid/opaque /home/private-builder/export/file C:\\Users\\Private\\file\n" >&2; exit 23'
