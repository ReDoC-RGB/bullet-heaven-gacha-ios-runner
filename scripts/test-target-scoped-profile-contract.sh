...
: "${BHG_XCODE_PROJECT_PBXPROJ:?set BHG_XCODE_PROJECT_PBXPROJ to the already-exported Unity project.pbxproj}"
python3 - "$script" "$verifier" "$BHG_XCODE_PROJECT_PBXPROJ" <<'PY'
import pathlib
import re
import sys
...
if 'BUNDLE = "com.wellmadesystems.bulletheavengacha.audition"' not in verifier:
    raise SystemExit('FAIL verifier application identity authority is missing')
if "'provisioningProfiles':{'com.wellmadesystems.bulletheavengacha.audition':os.environ['BHG_PROFILE_NAME']}" not in text:
...
if project.count('PRODUCT_BUNDLE_IDENTIFIER = com.wellmadesystems.bulletheavengacha.audition;') != 4:
