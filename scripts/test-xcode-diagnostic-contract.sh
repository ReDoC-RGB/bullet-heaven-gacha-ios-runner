...
ARCHIVE_DIAGNOSTIC_DIR="${RUNNER_TEMP}/bhg-diagnostic-archive-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}"
EXPORT_DIAGNOSTIC_DIR="${RUNNER_TEMP}/bhg-diagnostic-export-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}"
...
printf 'profile-bytes\n' > "${ROOT}/bullet-heaven-gacha.mobileprovision"
printf 'keychain-bytes\n' > "${ROOT}/bullet-heaven-gacha.keychain-db"
...
${GITHUB_WORKSPACE}/Source/File.mm:42:7: error: use of undeclared identifier 'BHGFixtureCompilerFailure'
Ld ${RUNNER_TEMP}/bhg-ios-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}/BulletHeavenGachaAudition normal arm64
Undefined symbols for architecture arm64:
  "_BHGFixtureLinkerFailure", referenced from:
...
CodeSign ${HOME}/Library/Developer/Xcode/DerivedData/BulletHeavenGachaAudition.app
...
grep -F "error: use of undeclared identifier 'BHGFixtureCompilerFailure'" "${ARCHIVE_DIAGNOSTIC_DIR}/xcodebuild-sanitized.log"
...
error: exportArchive: No profiles for 'com.wellmadesystems.bulletheavengacha.audition' were found
Authorization: Basic ${FIXTURE_TOKEN}
private export URL ${FIXTURE_URL}
private absolute path ${RUNNER_TEMP}/bhg-ios-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}/BulletHeavenGachaAudition.xcarchive
EOF
...
grep -F "error: use of undeclared identifier 'BHGFixtureCompilerFailure'" "${EXPORT_DIAGNOSTIC_DIR}/xcodebuild-sanitized.log"
...
grep -F "error: exportArchive: No profiles for 'com.wellmadesystems.bulletheavengacha.audition' were found" "${EXPORT_DIAGNOSTIC_DIR}/xcodebuild-sanitized.log"
